...
Hoje, convidar um novo perfil "agent" via `POST /api/v1/users/invite` cria um login `res.users` vinculado a um registro `thedevkitchen.estate.profile`. Um registro de domínio `real.estate.agent` **já existe** nesse ponto — `profile_api.py` (Feature 010) o cria automaticamente no momento em que o perfil é criado — mas fica **sem `user_id`**, e nenhum plano existente vincula esse registro de volta ao novo `res.users`, o campo que todo caminho de código de RBAC/notificação de fato lê. Esta feature unifica os dois caminhos de onboarding — `POST /api/v1/agents` (registro apenas, sem login) e `POST /api/v1/users/invite` (login apenas, registro órfão sem `user_id`) — em um único fluxo: quando o perfil por trás de um convite é do tipo `agent`, `POST /api/v1/users/invite` aceita um objeto aninhado `agent` que espelha o conjunto completo de campos de `AGENT_CREATE_SCHEMA` (`name`, `cpf`, `email`, `phone`, `mobile`, `creci`, `hire_date`, `bank_name`, `bank_account`, `pix_key` — excluindo apenas `company_id`/`user_id`, que são derivados no servidor), validado de forma idêntica a `POST /api/v1/agents`, e vincula atomicamente (via `write()` no registro já existente, ou `create()` no caso defensivo de não existir um — semântica de upsert, FR2.1) um registro `real.estate.agent` **tanto** ao perfil (`profile_id`) **quanto** ao novo login (`user_id`), na mesma transação do convite. `POST /api/v1/agents` é removida definitivamente (sem janela de descontinuação) assim que o fluxo unificado estiver implementado e validado.
