...
| `thedevkitchen_cms` | Application | Headless CMS for agency web pages (Puck JSON editor), media library, public SSR route for Next.js consumption. Feature 028 added a platform-level, non-tenant generic-template catalog (admin-curated via Odoo UI only, no REST write) with a curated-copy-into-tenant action — see [architecture.md](knowledge_base/architecture.md) §7. |
...
8. **Resolved: `.specify/memory/constitution.md` bumped to v1.11.0 for Feature 028.** Feature 028 (`028-cms-generic-templates`, fully implemented/tested/reviewed — "Ready to merge: Yes") introduced three reusable architectural patterns, now recorded in both this repo's knowledge base ([architecture.md](knowledge_base/architecture.md) §7, [modules-custom.md](knowledge_base/modules-custom.md) `thedevkitchen_cms`, [multi-tenancy.md](knowledge_base/multi-tenancy.md) isolation-mechanisms #6, [security.md](knowledge_base/security.md) admin-access-paths) and in `.specify/memory/constitution.md` v1.11.0's "Architectural Patterns" and "Reference Implementations" sections: (1) **Platform-Level Non-Tenant Catalog Entity** — no `company_id`, admin-curated via Odoo UI only, no `ir.rule` (nothing to isolate), access via `ir.model.access.csv` + controller role check; (2) **Curated-Copy-Into-Tenant Pattern** — a REST action that materializes a permanent, company-scoped snapshot from a platform entity with a `source_*_id` traceability field and no ongoing sync; (3) **Role-Set Reuse Across Sibling Controllers** — a shared RBAC-tuple constant imported (not redefined) by a new sibling controller, enforced as a fix during Feature 028's own final review (`GENERIC_TEMPLATE_MANAGEMENT_ROLES = TEMPLATE_MANAGEMENT_ROLES` in `cms_template_generic_controller.py`, importing from `cms_template_controller.py`). Also a reference instance of ADR-029 ("admin never authenticates via REST") applied to a new domain — no new ADR needed.
