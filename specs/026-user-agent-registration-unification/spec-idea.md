...
1. **Paridade total de campos para o nó `agent`.** O objeto aninhado `agent` em `POST /api/v1/users/invite` deve aceitar o MESMO conjunto de campos que `AGENT_CREATE_SCHEMA` (verificado literalmente em `quicksol_estate/controllers/utils/schema.py`, linhas 25-55) — não apenas `creci`/`hire_date`/`bank_name`/`bank_account`/`pix_key`, mas também `name`, `cpf`, `email`, `phone`, `mobile` — cada um validado com as mesmas restrições que `create_agent` já aplica (`name` com 3-255 caracteres, `cpf` com verificação de 11 dígitos após remover pontuação, `email` contendo `@`/`.`, `creci` com no mínimo 4 caracteres). Os únicos campos excluídos do nó são `company_id` e `user_id`, que permanecem derivados no servidor (a partir da empresa do perfil já vinculado e do login recém-criado, respectivamente) — aceitar qualquer um deles vindo do cliente reabriria o vetor de spoofing entre empresas/usuários que a ADR-008 existe para fechar. Essa exclusão foi confirmada via `AskUserQuestion` durante a elaboração da spec (2026-07-18): "excluir + derivar no servidor" foi escolhido em vez de "aceitar e cruzar-validar" ou "aceitar e confiar", para preservar a ADR-008 e ainda assim satisfazer o requisito de paridade total de campos para todos os outros campos.
...
Hoje, convidar um novo perfil "agent" via `POST /api/v1/users/invite` cria um login `res.users` vinculado a um registro `thedevkitchen.estate.profile`, mas — diferente de `POST /api/v1/agents` — nunca cria o registro de domínio `real.estate.agent` correspondente (licença CRECI, dados bancários/de comissão), e mesmo que criasse, nenhum plano existente vincula esse registro de volta ao novo `res.users` via `user_id`, o campo que todo caminho de código de RBAC/notificação de fato lê. Esta feature unifica os dois caminhos de onboarding — `POST /api/v1/agents` (somente registro, sem login) e `POST /api/v1/users/invite` (somente login, sem registro) — em um único fluxo: quando o perfil por trás de um convite é do tipo `agent`, `POST /api/v1/users/invite` aceita um objeto aninhado `agent` que espelha o conjunto completo de campos de `AGENT_CREATE_SCHEMA` (`name`, `cpf`, `email`, `phone`, `mobile`, `creci`, `hire_date`, `bank_name`, `bank_account`, `pix_key` — excluindo apenas `company_id`/`user_id`, que são derivados no servidor), validado de forma idêntica a `POST /api/v1/agents`, e cria atomicamente um registro `real.estate.agent` vinculado **tanto** ao perfil (`profile_id`) **quanto** ao novo login (`user_id`), na mesma transação do convite. `POST /api/v1/agents` é removida definitivamente (sem janela de descontinuação) assim que o fluxo unificado estiver implementado e validado.
...
**FR2: Criação Atômica do Registro de Agente Durante o Convite (vinculado tanto ao perfil quanto ao login)**
- FR2.1: Quando o tipo do perfil-alvo é `agent`, `invite_user` DEVE criar um registro `real.estate.agent` (via `profile_id=profile_record.id` mais quaisquer campos fornecidos no objeto `agent`) na MESMA transação de banco de dados que a criação do `res.users` e a geração do token de convite.
- FR2.1b (**novo, corrige a lacuna da Feature 025**): O registro `real.estate.agent` criado TAMBÉM DEVE ter `user_id` definido para o `res.users.id` recém-criado. Isso é obrigatório, não opcional — todo caminho de código de RBAC/notificação que restringe dados ao "agente atualmente logado" (`property_api.py`, `lead_api.py` ×3, `serializers.py`, `proposal.py` ×4, `record_rules.xml`) lê `agent_id.user_id`, nunca `agent_id.profile_id`.
- FR2.2: Se a criação de `real.estate.agent` falhar por qualquer motivo (validação, restrição de banco — incluindo as restrições já existentes `_check_user_unique` e `_check_creci_format`), a requisição inteira DEVE falhar atomicamente — nenhum `res.users`, nenhum token de convite, nenhum e-mail enviado (rollback implícito de transação do Odoo, consistente com o padrão "Atomic Dual Record Creation" da constituição).
...
- Campos existentes relevantes usados por esta feature: `profile_id` (Many2one → `thedevkitchen.estate.profile`, já aciona a sincronização `setdefault()` do `create()` descrita abaixo), `user_id` (Many2one → `res.users`, "Link to user account if agent has system access" — **atualmente não definido em nenhum lugar do fluxo de convite; esta feature é o que começa a preenchê-lo**), `creci`, `creci_normalized` (computado, armazenado, indexado), `bank_name`, `bank_account`, `bank_account_type`, `pix_key`, `hire_date`, `company_id` (FK obrigatória, multi-tenancy).
...
**Nova superfície de validação/orquestração (nível de controller, mais a única migração em nível de campo acima) — o controller apenas repassa dados, o reaproveitamento acontece no `create()` do modelo (FR1.4c)**:
```python
...
if profile_type == "agent":
    ...
    agent_vals = {
        "profile_id": profile_record.id,  # <-- basta isso para acionar o reaproveitamento (FR1.4c)
        "user_id": user.id,  # <-- a correção: vincula ao login, não só ao perfil
    }
    if agent_payload:
        ...
        # nada além disso é necessário: qualquer campo repetido (name/cpf/email/phone/mobile/
        # hire_date) que NÃO esteja em agent_vals neste ponto será preenchido automaticamente
        # por create()'s vals.setdefault(...) a partir do profile_id acima — não duplicar aqui.
    agent = request.env["real.estate.agent"].sudo().create(agent_vals)  # gera exceção em conflitos de CRECI/usuário -> reverte toda a requisição
```
