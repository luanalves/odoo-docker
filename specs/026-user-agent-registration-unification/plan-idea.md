...
- **⚠️ Achado 2026-07-20 — restart obrigatório para mudanças de CONTROLLER testadas via curl**: o comando acima (`-u <módulo> --stop-after-init`) roda um processo Odoo *separado e efêmero* que só atualiza o banco (schema/dados de módulo) e depois encerra — ele NÃO é o processo que atende as requisições HTTP na porta 8069 (esse é o container `odoo18`, de longa duração, já rodando). Para mudanças em arquivos `.py` de **controllers** (ex.: `invite_controller.py`) serem refletidas nas respostas HTTP que os scripts `integration_tests/*.sh` batem, é necessário **reiniciar o container** depois do upgrade: `docker compose restart odoo` (a partir de `18.0/`), e aguardar até `curl -s -o /dev/null -w "%{http_code}" http://localhost:8069/web/login` retornar `200` antes de rodar o script. Sem isso, o script vai bater na versão antiga do controller em memória e os testes vão falhar de forma confusa (parece que o código não funcionou, mas na verdade nunca foi recarregado). Mudanças em **models**/**schema.py** consumidas pelos testes unitários (`--test-tags`/`run_unit_tests.py`) não têm esse problema, pois cada execução de teste sobe seu próprio processo Python do zero.
